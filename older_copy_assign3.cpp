...
vector<double> trace(double o[3], double d[3])
{
   bool intersectTriangle = false, intersectSphere = false;
   double bcoords[3], other1[3];
   double intersection[3] = {1000.0, 1000.0, 1000.0};
   int index = 0;
   for (int i = 0; i < num_triangles; i++) if (intersectsTriangle(triangles[i], o, d, intersection, bcoords))
   {
      intersectTriangle = true;
      index = i;
      if (done) printV("intersection", intersection);
      if (done) printV("bcoords", bcoords);
   }
   for (int i = 0; i < num_spheres; i++) if (intersectsSphere(spheres[i], o, d, intersection))
   {
      intersectSphere = true;
      index = i;
   }

   if (!intersectTriangle && !intersectSphere) return {1.0, 1.0, 1.0};
   // printV("intersection", intersection);
   vector<double> illumination = {ambient_light[0], ambient_light[1], ambient_light[2]};

   for (int j = 0; j < num_lights; j++)
   {
      double l[3], n[3], n1[3], v[3], r[3], diffuse[3], specular[3], shiny;
      for (int i = 0; i < 3; i++)
      {
      ...
      if (intersectSphere)
      {
         if (done) cout << "intersected circle" << endl;
         for (int i = 0; i < 3; i++)
         {
      ...
      // if (shadow && intersectTriangle && done)
      // if (shadow && intersectSphere) cout << "found sphere with a shadow " << endl;
      if (!shadow)
      {
         normalize(l);
         normalize(n);
   ...
   if (done) cout << illumination[0] << " " << illumination[1] << " " << illumination[2] << endl;
   return illumination;
}
...
void draw_scene()
{
  ...
  for(x=0; x<WIDTH; x++)
  {
    ...
    for(y=0;y < HEIGHT;y++)
    {
      ...
      vector<double> color = trace(origin, direction);
